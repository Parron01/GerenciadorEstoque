...
import type { Database } from "better-sqlite3"; // Add this import
...
const db: Database = BetterSqlite3(dbPath);
...
function initDatabase() {
  ...
  if (!adminUser) {
    ...
    if (adminUsername && adminPassword) {
      ...
      console.error(
        "ADMIN_USERNAME ou ADMIN_PASSWORD não definidos no arquivo .env"
      );
    }
  }

  // Inserir dados padrão se tabela de produtos estiver vazia
  const result = db.prepare("SELECT COUNT(*) as count FROM products").get() as {
    count: number;
  };
